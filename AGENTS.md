...
- OpenSSH SSHSIG (`ssh-keygen -Y sign/verify`) is approved only for the explicit Phase-37 schema-v2 blind-review authenticity commands. Keep private signing keys, `allowed_signers` trust roots and revocation files outside project persistence. Sign and verify exact completed-ledger bytes under the fixed project namespace; never reserialize before signing, never silently fall back to unsigned verification, and never treat a valid reviewer signature as literary approval or production provider admission.
...
For Phase 37 reviewer-ledger-authenticity changes, run the dedicated `Phase 37 Reviewer Ledger Authentication` workflow. It must exercise real local OpenSSH SSHSIG signing/verification on rights-safe synthetic ledger data on Linux and Apple Silicon, prove exact-byte tamper rejection, wrong-principal rejection and KRL revocation, and keep legacy unsigned evidence separate. Never commit generated private keys, reviewer trust files, signatures from real reviews, manuscripts or private reviewer text. Do not describe a valid reviewer-ledger signature as authenticity of the reveal-key Candidate A/B mapping; that authority/provenance boundary remains separate.
