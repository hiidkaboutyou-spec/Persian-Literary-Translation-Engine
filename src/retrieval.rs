...
impl LiteraryMemory {
    ...
    pub fn add_translation(&mut self, entry: TranslationMemoryEntry) { self.translation_memory.push(entry); }
    pub fn add_glossary_entry(&mut self, entry: GlossaryEntry) { self.glossary.push(entry); }
    pub fn add_character(&mut self, character: Character) { self.characters.push(character); }
    pub fn add_relationship(&mut self, relationship: RelationshipMemory) { self.relationships.push(relationship); }
    ...
    pub fn search_translation_memory(&self, query: &str, limit: usize) -> Vec<ScoredMemory<'_>> {
        ...
        let mut hits: Vec<_> = self.translation_memory.iter().filter_map(|entry| {
    ...
    pub fn glossary_for_text(&self, source_text: &str) -> Vec<&GlossaryEntry> {
        ...
        self.glossary.iter().filter(|entry| contains_phrase(&normalized, &normalize(&entry.source))).collect()
    }

    pub fn characters_for_text(&self, source_text: &str) -> Vec<&Character> {
        let normalized = normalize(source_text);
        self.characters.iter().filter(|character| {
            contains_phrase(&normalized, &normalize(&character.name)) || character.aliases.iter().any(|alias| contains_phrase(&normalized, &normalize(alias)))
        }).collect()
    }

    pub fn relationships_for_characters<'a>(&'a self, character_names: &[&str]) -> Vec<&'a RelationshipMemory> {
        let names: HashSet<String> = character_names.iter().map(|name| normalize(name)).collect();
        self.relationships.iter().filter(|relationship| {
            names.contains(&normalize(&relationship.character_a)) && names.contains(&normalize(&relationship.character_b))
        }).collect()
    }
...
fn normalize(text: &str) -> String {
    text.to_lowercase().chars().map(|ch| if ch.is_alphanumeric() { ch } else { ' ' }).collect::<String>().split_whitespace().collect::<Vec<_>>().join(" ")
}

fn contains_phrase(haystack: &str, needle: &str) -> bool { !needle.is_empty() && haystack.contains(needle) }
...
fn text_similarity(a: &str, b: &str) -> f32 {
    ...
    let substring_bonus = if a_norm.contains(&b_norm) || b_norm.contains(&a_norm) { 0.25 } else { 0.0 };
...
mod tests {
    ...
    fn retrieves_similar_translation_memory() {
        ...
        memory.add_translation(TranslationMemoryEntry { source: "a quiet smile".into(), translation: "لبخندی آرام".into(), context: "gentle dialogue".into() });
